...
export default function Selection({ onGenerate }: SelectionProps) {
  ...
  const fetchCaseList = async (difficulty: string) => {
    ...
    try {
      const response = await fetch('https://nonvegetative-may-untensile.ngrok-free.dev/api/generate-case-list', {
        method: 'POST',
  ...
  if (!selectedDifficulty) {
    return (
      <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#05070a]">
        <DotGrid proximity={120} baseColor="rgba(255, 255, 255, 0.03)" resistance={650} returnDuration={1} />
      ...
      <div className="relative z-20 w-full max-w-6xl h-[80vh] flex flex-col md:flex-row bg-black/60 backdrop-blur-xl border border-white/10 shadow-2xl">
        ...
        {isLoadingList && (
          <div className="absolute inset-0 z-30 bg-black/90 flex flex-col items-center justify-center">
            ...
            <p className="text-[14px] text-purple-500 font-bold tracking-[0.3em] animate-pulse uppercase">
              Scanning Global Case Database...
            </p>
            <p className="text-[12px] text-purple-500/50 mt-2 font-mono">
          ...
          >
            {!isLoadingList && caseDatabase.length > 0 && caseDatabase.map((c, index) => (
              <div
                ...
                className={`flex h-36 cursor-pointer border-b border-white/5 transition-all duration-300 ${selectedCase?.id === c.id ? 'bg-purple-900/20 border-l-4 border-l-purple-500' : 'hover:bg-white/5'
                  }`}
              >
            ...
            <>
              <div>
                <div className="flex justify-between items-center mb-6">
                  <span className="text-[10px] text-purple-500/50 font-bold uppercase tracking-[0.5em]">Target Profile</span>
                  <span
                    className="text-[10px] font-black tracking-widest uppercase px-4 py-1 border border-transparent"
                    style={{
                      color: modes.find(m => m.id === selectedDifficulty)?.color || '#22d3ee',
                      boxShadow: `0 0 15px ${modes.find(m => m.id === selectedDifficulty)?.color.slice(0, 7)}40` //Added a subtle neon glow
                    }}
                  >
                    {selectedDifficulty}
                  </span>
                ...
                <div className="relative w-full h-75 border border-white/10 mb-8 overflow-hidden group">
                ...
                <p className="text-[13px] text-white/50 uppercase tracking-widest leading-relaxed border-l-2 border-purple-500 pl-4">
                  {selectedCase.desc}
                </p>
              </div>
              ...
              <div className="flex gap-4 mt-8">
                <button
                  ...
                  className="px-6 py-4 text-[13px] font-bold text-white/30 border border-white/40 hover:bg-white/5 hover:text-white transition-all uppercase tracking-widest"
                >
                  Abort
                </button>
                <button
                  onClick={() => onGenerate(selectedCase.title, selectedDifficulty!, selectedCase.keyword, selectedCase.desc)}
                  className="flex-1 px-6 py-4 bg-purple-500/10 text-purple-500 border border-purple-500 font-bold text-[13px] uppercase tracking-[0.3em] hover:bg-purple-500 hover:text-black hover:shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all duration-300"
                >
